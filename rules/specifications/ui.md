...
関心の分離により、担当者が必要な仕様だけを参照、更新できる。巨大な単一ファイルは検索、レビュー、差分管理が崩壊する。
...
仕様書のライフサイクル（誰が、いつ、何のステータスで書いたか）が追跡できなければ、どれが最新で誰が承認したかが不明になる。`related`で依存関係を明示することで影響範囲調査が可能になる。
