...
本書は、UI仕様書を新規作成または変更する場合に適用する。本書は、[仕様書共通規則](specification.md)を前提とする。

UI仕様書は、どのプロジェクト、チーム、フェーズでも「唯一の真実のソース（Single Source of Truth）」として機能し、認識齟齬、手戻り、属人化を排除する。見た目を説明する資料ではなく、画面に表示する情報、ユーザー操作、状態遷移、入力規則、例外処理、権限制御、レスポンシブ対応、アクセシビリティ、受け入れ条件を定義する正式な仕様書である。

---

## 2. UI仕様書の原則

仕様書共通規則の原則に加え、UI仕様書には次を適用する。曖昧な表現の禁止、観測可能な挙動の記述など、仕様書共通規則に既に定義されている原則はここに再掲しない。
...
UI仕様書には、[API仕様書](api-specification.md)など他の仕様書の内容を全文重複記載しない。UIに影響する条件だけを記載し、詳細は正式な参照先へリンクする。
...
すべての仕様書ファイルの冒頭に、仕様書共通規則が定める4項目（バージョン、ステータス、最終更新日、責任者）を含む次のヘッダーを記述する。
...
`owner`は仕様の最終決定権を持つ人物とする。作成者と決定権者が異なる場合は決定権者を記載する。`related`で依存関係を明示することで影響範囲調査が可能になる。

### ステータス定義

`status`は仕様書共通規則が定める`draft` / `review` / `stable` / `deprecated`を使用する。

| ステータス | 意味 | 次のアクション |
|---|---|---|
| `draft` | 作成中、未レビュー | レビュー依頼 |
| `review` | レビュー中（PR作成済み） | レビュー指摘の対応 |
| `stable` | 承認済み、実装可能 | 実装開始 |
...
状態を持つ画面、コンポーネントには状態遷移表を必須とする。現在の状態とイベントの組み合わせを総当たりで検討し、想定しない組み合わせにも「遷移なし」など結果を明記する。図だけで状態遷移を記述すると、検討していない組み合わせが欠落したまま見過ごされやすい。表が状態遷移の正とし、Mermaidの`stateDiagram-v2`による図は表の内容を視覚的に把握するための補足として任意で添える。

**状態遷移表の例：**

| 現在の状態 | イベント | 条件 | 遷移先 | アクション |
| --- | --- | --- | --- | --- |
| Initial | 画面を開く | — | Loading | 一覧取得APIを呼び出す |
| Loading | 取得成功 | 件数 > `0` | Loaded | 一覧を表示する |
| Loading | 取得成功 | 件数 = `0` | Empty | 空状態を表示する |
| Loading | 取得失敗 | — | Error | `MSG-NETWORK-001`と再試行ボタンを表示する |
| Loaded | 行を選択 | — | Loaded | `SCR-USER-002`へ遷移する |
| Error | 再試行を押す | — | Loading | 一覧取得APIを再度呼び出す |
| Loaded、Empty、Error | 画面を開く（再訪） | — | Loading | 遷移なし。既存の状態遷移と同じ扱いにする |
...
stateDiagram-v2
    ...
    Loading --> Loaded : 取得成功（件数 > 0）
    Loading --> Empty : 取得成功（件数 = 0）
    Loading --> Error : 取得失敗
    Error --> Loading : 再試行
```

### 7-6. Mermaidで画面遷移図を記述する

画面遷移、複数システム間の処理順序はMermaidで記載する。
...
色、余白、角丸、影、文字サイズをUI仕様書へ直接記述しない。[Webデザイン基盤仕様書](../design/web-ui.md)が定義するデザイントークン名で参照し、そこに存在しないトークン名を独自に作らない。トークン運用を徹底しているデザインシステムほど、コンポーネントへ生の数値を直接指定することを避け、必ずトークン経由で参照させる。1つの生の数値が紛れ込むと、次第に近似した値が増え、デザインシステムの一貫性が崩れていくため。デザインツール（Figmaなど）が生の数値の入力自体を禁止しない場合でも、この規則はデザインファイルの中身ではなく、仕様書という正式な契約に何を記述するかを定める。個別画面でトークンを使わず直接値を使用する場合は、その理由を記載する。
...
トークン名で記述することで、デザインシステムの変更が仕様書の記述を変えずに全体に波及する。数値の直接記述は変更漏れの原因になる。
...
個別画面仕様は `screens/*.md` として独立ファイルで管理する。新規作成時は次のセクション構成に従う。
...
| 8 | 状態遷移表 | 必須 | 現在の状態、イベント、条件、遷移先、アクションを表で記述する。Mermaid図は任意で添える |
...
| 16 | 変更履歴 | 必須 | バージョン、日付、変更者、変更内容、承認者を記載する |
| 17 | 対象外 | 推奨 | 検討したが対象外とした項目と理由を記載する |
...
| 必須セクション | 個別画面仕様に要否「必須」のセクションが存在することを確認します。 |
...
```
[ ] ヘッダー（id / status / version / owner / last_updated）が揃っているか
...
[ ] 表示項目、入力項目に初期値が定義されているか
[ ] 状態遷移表が状態とイベントの組み合わせを網羅しているか（図だけになっていないか）
