...
本書は、UI仕様書の作成規則である。[仕様書共通規則](specification.md)を前提とする。
...
すべてのUI仕様書ファイルは、仕様書共通規則が定める4項目（バージョン、ステータス、最終更新日、責任者）に加え、次を先頭に記述する。

| 項目 | 内容 |
| --- | --- |
| タイトル | 対象の名称。 |
| レビュー担当 | レビューする役割または人物。複数の観点（デザイン、フロントエンドなど）がある場合は観点ごとに挙げる。 |
| 関連 | 依存または関連する画面、コンポーネントの識別子。影響範囲調査に使う。 |

`責任者`は仕様の最終決定権を持つ人物とする。作成者と決定権者が異なる場合は決定権者を記載する。

`ステータス`は仕様書共通規則が定める`draft` / `review` / `stable` / `deprecated`を使用する。`deprecated`は削除せず、後継仕様書へのリンクを追記する。過去の設計判断の根拠を残すため。
...
| 必須セクション | 採用したセクション構成のうち要否「必須」のセクションが存在することを確認します。 |
