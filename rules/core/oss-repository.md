...
コードコメントやREADMEで扱いきれない独立した文書を`docs/`へ配置する。読み手の目的に応じて[Diátaxis](https://diataxis.fr/)の4分類に、意思決定記録（ADR）を加えた区分で構成する。
...
すべてのプロジェクトが5分類すべてを必要とするわけではない。ドキュメントが少ない段階では`docs/`直下に置き、種類ごとの分割が必要になった時点でディレクトリを作る。
