...
| `docs/specifications/` | 契約の定義 | API、UI、通信プロトコル、インフラ・ネットワーク、アクセス制御、ハードウェアインターフェース、要求、安全要求など、実装が従うべき正式な契約を記述する文書。共通の記法は[仕様書共通規則](../specifications/common.md)で定め、専用規則がある領域はそれも適用する（[API仕様書作成ルール](../specifications/api.md)、[UI仕様書作成規約](../specifications/ui.md)）。 |
