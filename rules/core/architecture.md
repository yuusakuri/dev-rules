...
> 適用対象: ソースコードの設計、実装、レビューを行うすべての変更。
...
業務ロジックを持つコードは、実際のDB接続、ネットワーク通信、ファイルI/O、UIフレームワークを用意しなくても単体テストで検証できる構造にする。テストのしやすさは実装後に確認する結果ではなく、設計段階で満たす制約として扱う。純粋な処理と副作用の分離は「副作用は境界に閉じ込める」に、非決定的要素の差し替えは「実行環境に依存する処理を差し替え可能にする」に従う。

UIのbuild処理、HTTPハンドラーなど、フレームワークへ強く依存し単体テストがしにくいコードは、業務ロジックをできる限り持たせず、テスト可能な処理へ委譲するだけの薄い層（Humble Object）にする。
...
Type、Interface、Class、Functionなどのプログラミング言語上の構成要素を理由にファイルを分割・集約しない。`types.ts`、`interfaces/`、`models/`のように言語要素で束ねたファイルは、無関係なドメインの型が同じファイルへ混在し、変更のたびに無関係な差分が発生する。

型定義は、それが属するドメイン・責務・機能ごと（例: `user`、`payment_gateway`）のファイルに分割するか、その型を最も強く所有するロジックと同じモジュールに配置する。
...
依存する機能やオブジェクトは、関数の引数または型のフィールドとして外部から明示的に渡す依存性の注入を採用する。ただし、DIフレームワークは極力使用しない。グローバル状態やサービスロケータのように依存関係を隠す仕組みは、処理の理解とテストを難しくするため避ける。これにより、変更すべき部分を特定しやすくなり、テスト用の実装へ差し替えやすくなる。
...
同じ処理が複数箇所に重複すると、変更時にすべての箇所を修正する必要が生じ、修正漏れがバグになる。重複を関数化、共通化することで変更箇所を1か所に集約する。ただし早すぎる共通化は YAGNI に反する。同じコードが3か所以上に現れてから共通化を検討する。
...
### バイナリデータを決定性有限オートマトンで逐次解析する

ストリームや分割された入力からバイナリデータを解析する場合は、Deterministic Finite Automaton（決定性有限オートマトン）として解析状態と遷移条件を明示し、1回の呼び出しで1バイトだけを受け取って状態を進める関数をループから呼び出す構造を積極的に採用する。解析途中の状態を保持し、Frameが完成した時点で結果を返すことで、入力の分割位置に依存せず、固定長バッファとストリームを同じ解析器で扱える。これにより、状態ごとの単体テスト、異常入力からの復帰、メモリ使用量の制御が容易になるが、全データを一括処理する方が明らかに単純な場合には無理に適用しない。
...
| 外部接続の命名 | 外部通信やインフラストラクチャの処理を、`api`、`data`、`infrastructure` のような抽象的な技術概念で命名・集約しない。実際の接続先システム、サービス、通信対象を明示する（例: `main_backend`、`stripe_payment`、`device_hub`）。 |
...
- ログはエラーが発生した場所で1回だけ記録する。伝播のたびにログしない
...
- ログはエラーが発生した場所で1回だけ出力する（伝播のたびに出力しない）
