...
Next.js App RouterやExpo Routerのように、フレームワークが`app/`配下の全ファイルをルーティング対象として自動検出する場合、ルートではないComposition Rootのコード（プロバイダーの組み立てなど）を`app/`へ置くと誤検出される。この場合は`<source-root>/app/`をフレームワークのルーティング専用とし、Composition Rootは`<source-root>/bootstrap/`へ配置する。
