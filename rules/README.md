...
| 仕様書 | [仕様書共通規則](specifications/common.md) | 領域を問わず仕様書（API、UI、通信プロトコル、インフラ・ネットワーク、アクセス制御、ハードウェアインターフェース、要求、安全要求など）を新規作成または変更する。専用規則がない領域ではこの規則だけを適用する。 | なし |
| 仕様書 | [API仕様書](specifications/api.md) | API仕様書を新規作成または変更する。 | 仕様書共通規則。実装も変更する場合は該当する共通・実装規則 |
| 仕様書 | [UI仕様書](specifications/ui.md) | UI仕様書を新規作成または変更する。 | 仕様書共通規則。Web UIが対象ならWeb UI規則。実装も変更する場合は該当する共通・実装規則 |
...
| API仕様書だけを作成 | 仕様書共通規則 → API仕様書 |
| WebのUI仕様書だけを作成 | 仕様書共通規則 → Web UI → UI仕様書 |
| UI仕様書に基づいてWeb UIを実装 | 共通設計原則 → アプリケーション設計規則 → 該当する実装規則 → Web UI → 仕様書共通規則 → UI仕様書 |
| 専用規則がない領域の仕様書を作成（通信プロトコル、インフラ・ネットワーク、アクセス制御、ハードウェアインターフェース、要求、安全要求など） | 仕様書共通規則 |
