...
この文書は、特定のUIライブラリやフレームワークに依存しない、UIの視覚表現、レイアウト、状態、動きのガイドラインを定める。
...
本書ではトークンを識別するため、`color.fg.default`のように`.`で階層を区切って表記する。この表記は仕様書上の識別形式であり、実装上の名前を指定するものではない。
