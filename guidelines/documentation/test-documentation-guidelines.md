...
解説記事では、テスト実行後に記載する情報として、次の項目を説明している。

| 項目 | 内容 |
| --- | --- |
| 結果判定 | テストケースの結果をOK、NGなどで記載する。 |
| 実行日 | テストを実行した日付を記載する。 |
| 実行者 | テストケースを実行した担当者を記載する。 |
| 備考 | アカウント情報、申し送り事項、不具合票の番号など、結果を後から確認する際に必要な情報を記載する。 |

配布Test Caseテンプレートの結果記載欄には、次の項目がある。
...
本テンプレートでは、これらの結果項目をTest Logへ記録する。

結果のステータスは、Test Planのテスト実施結果の判定基準を参照する。

### 9.2 集計欄

配布Test Caseテンプレートには、結果件数や進捗率などの集計欄がある。
