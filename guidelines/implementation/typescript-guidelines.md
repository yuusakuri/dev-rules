...
| `apps/<app-name>/src/core/<capability>/` | `apps/myproject-web/src/core/clock/clock.ts` | 通信を伴わずに実行環境から得る値（時刻、識別子の発番、乱数など）の契約と実装を配置する。 |
...
| `apps/<app-name>/src/features/<feature>/connectors/` | `apps/myproject-web/src/features/payment/connectors/stripe_client.ts` | ネットワーク越しの外部サービスを利用する契約、接続先別の実装、外部データ形式との変換を配置する。 |
