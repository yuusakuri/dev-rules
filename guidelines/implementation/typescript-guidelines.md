...
| `apps/<app-name>/src/features/<feature>/connectors/` | `apps/myproject-web/src/features/payment/connectors/stripe_payment_client.ts` | 永続化以外の外部システム、外部資源と通信する契約、接続先別の実装、外部データ形式との変換を配置する。フォルダ名は境界の置き場所を表す。契約の型名は「共通設計原則」の「命名」に従う。 |
