...
| `apps/<app-name>/src/features/<feature>/connectors/` | `apps/myproject-web/src/features/payment/connectors/stripe_client.ts` | 外部資源（外部システム、通知、デバイス、時刻など）を利用する契約、接続先別の実装、外部データ形式との変換を配置する。 |
