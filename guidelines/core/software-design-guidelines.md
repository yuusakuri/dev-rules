...
| `subscriber` | 受け取りたい対象を発行側へ登録し、配信されるメッセージを受け取る型に使用する。受け取った後の処理は`handler`が担う。関連: `listener`、`handler`も参照。 | [`async_nats`の`Subscriber`](https://docs.rs/async-nats/latest/async_nats/struct.Subscriber.html)、[`zenoh`の`Subscriber`](https://docs.rs/zenoh/latest/zenoh/pubsub/struct.Subscriber.html) | 名詞 | 通信 |
