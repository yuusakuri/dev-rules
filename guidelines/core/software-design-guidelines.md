...
| `listener` | 接続要求を待ち受け、確立した通信路を受け取る型に使用する。資源や状態の変化を監視する型は`watcher`を使用する。関連: `server`も参照。 | [`TcpListener`](https://doc.rust-lang.org/std/net/struct.TcpListener.html)、[`tokio`の`TcpListener`](https://docs.rs/tokio/latest/tokio/net/struct.TcpListener.html) | 名詞 | 通信 |
