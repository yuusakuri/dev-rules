...
本書は、言語やフレームワークに依存しない設計原則を定義する。他人が読んでも意図が分かり、変更しても壊れる範囲が予測できるコードを保つことを目的とする。
...
保守しやすさは「何をどこに置くか」で決まる。本節では、業務ロジックを中心に据え、その周りへ外部技術を配置する分け方を扱う。
...
「このクラスを書き換えるのはどんなときか」を自問し、料金計算の変更と帳票レイアウトの変更など、性質の異なる答えが複数出る場合は責務を分割する。
...
Type、Interface、Class、Functionなどのプログラミング言語上の構成要素を理由に、ファイルを分割、集約しない。`types.ts`に全型を集める構成は、1つの機能を追うたびに複数ファイルを行き来させる。
...
依存関係は、変更が伝わる経路である。本節では、その経路を一方向に保ち、必要な箇所だけ差し替えられるようにする方法を扱う。
...
あるインターフェースの実装を別の実装へ差し替えても、呼び出し元の動作が壊れてはならない。この保証があって初めて、Dependency Inversionは機能する。

実装が事前条件、事後条件、不変条件を破ると、呼び出し元は特定の実装でしか動かない書き方に暗黙のうちに依存し、抽象は形だけのものになる。
...
外部I/O、永続化、時刻、ID生成、乱数など、実行のたびに結果が変わり得るものは、呼び出し元から差し替えられる形で抽象化する。直接埋め込むと、テストに外部サービスやDBが必要になり、月末だけ落ちるような不具合も再現できない。抽象化すれば、時刻やIDをテスト用の固定値へ置き換えて検証できる。

### 依存関係を明示的に渡す

依存する機能やオブジェクトは、関数の引数または型のフィールドとして外部から明示的に渡す（依存性の注入）。グローバル状態やサービスロケータなど依存を隠す仕組みは、何を使っているかを読んでも分からなくし、テストでの差し替えも難しくするため避ける。
...
lintで検出し、依存の向きを見直す。両者が必要とする共通部分は、双方から依存される下位モジュールとして切り出す。
...
コードは書く時間より読む時間のほうが長い。本節の規則は、読む人が一度に覚えておく量を減らすために置く。

### 必要になるまで作らない（YAGNI）

将来使うかもしれない機能、抽象化、汎用化は、今は作らない。使われない実装も保守の対象になり、読む人には「どこかで使われているはず」の探索対象になる。実際に要件が来たときには、想定と違っていることも多い。
...
まとめる対象は「同じ知識」であり、「たまたま似ているコード」ではない。別々の理由で変わるコードを共通化すると、片方の都合で条件分岐が増えていく。

### 同じ情報を複数の状態として持たない

他の状態から計算できる値は、独立した状態として保存せず、必要な時点で導出する。同じ情報の写しが複数あると、片方だけ更新されて食い違う。情報源が1つなら、不整合そのものが起きない。

### 隣の相手までしか知らない（Law of Demeter）

オブジェクトの処理内で操作してよいのは、自分自身、自身が直接保持する変数、渡された引数、その処理内で生成したオブジェクトに限る。`a.b().c().d()`のように連鎖して奥へ入り込むと、途中の型の内部構造すべてに依存することになる。必要な結果は、直接の相手に依頼して受け取る。
...
本節では、1つのモジュールやクラスの区切り方と、外へ見せる範囲を扱う。公開した部分は利用側が依存するため、後から狭めるのは難しい。
...
入力値の制約や取り得る状態は、コメントや実行時チェックに頼らず型で表現し、無効な値や不正な状態遷移をコンパイル時または生成時に防ぐ。型で表せていれば、確認するのは型定義1か所で済む。
...
この型は生のコレクション操作を隠し、許可された操作と検証を一か所へまとめる。生のまま扱うと、追加する箇所ごとに同じ検証を書くことになり、いずれ書き漏れる。
...
パターンは、繰り返し現れる問題に名前と定型の解法を与えたものである。問題が起きていないのに当てはめると、複雑さだけが増える。今どの問題に当たっているかを確かめてから使う。
...
名前は、読む人が最初に受け取る仕様である。中身を読まなくても役割が分かれば、コードを追う量はそれだけ減る。

本節は2つの表からなる。「規則」は名前を決める判断基準、「単語」はよく使う語の意味を固定した辞書である。命名に迷ったら単語表を先に引き、該当する語があればその意味どおりに使う。

### 規則

| ルール | 内容 |
| --- | --- |
| 名前の具体性 | 名前だけで役割、対象、処理内容が推測できるようにし、接続先、扱うデータ、責務を名前に含める。`Abstract`、`Base`、`Common`、`Shared`、`Manager`、`Helper`、`Process`、`Util`、`Object`、`Raw` のような汎用名は、何をするものかを説明しないため使わない。 |
...
| 省略、略語 | 独自の略語は、読み手が意味を推測できないため使わない。業界標準の略語は使用してよい。NG: `tbl`、OK: `table`、`uuid` |
| 短く命名する | 文脈から明らかな語は省く。`UserService` の中の `userName` は `name` でよい。 |
| 抽象と具体、インターフェース | 抽象側（インターフェース）には汎用的、概念的な名前、具体側（実装）には詳細、技術的な名前を付ける。インターフェースは能力、役割を表す名詞または形容詞にする。`I` プレフィックスと `Impl` サフィックスは、実装の区別を役割ではなく記号で表すため使わない。NG: `IOrderRepository` / `OrderRepositoryImpl`、OK: `OrderRepository` / `PostgresOrderRepository` |
| データ構造 | 内部の処理状態を名前に含めず、ドメイン名を付ける。処理段階は扱う側の都合であり、データ自体の性質ではない。NG: `ParsedMessage`、`RawFrame`、OK: `Message`、`Frame` |
...
型名や関数名に使う語は、次の表の意味で統一する。「品詞」と「タグ」は、目的に近い語を探す手がかりとして付けている。
...
エラー処理の目的は、失敗を握りつぶさず、対処できる場所まで届けることにある。

エラー型はモジュール、レイヤーごとに定義し、上位へ渡すときにそのレイヤーの型へ変換して伝播させる。呼び出し元は、下位の実装が使うエラーの詳細を知らずに済む。

エラーの強制的な無視は禁止する。呼び出し元が戻り値やエラーコードを必ず確認できる設計にする。
...
並行処理の不具合は再現しにくく、原因の特定に時間がかかる。速さを求める前に、待ちと共有の扱いを決める。
...
- スレッド、タスク間でのメモリ共有はできる限り避け、メッセージパッシングを優先する。共有する状態がなければ、競合も起きない
...
ログは、後から問題を調べる手がかりである。量が多いほど分かるわけではなく、必要な1行が埋もれれば価値は下がる。
