...
型が満たすべき条件は、その型の生成と操作の内側で検証し、外から不正な値や状態を作れないようにする。値の範囲や形式、要素の重複、並び順、件数、取り得る状態と遷移は、いずれも同じ扱いとする。

条件を型で表現できる場合はコンパイル時に、できない場合は生成時に検証する。利用側の実行時チェックやコメントに委ねない。
