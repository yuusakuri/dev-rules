...
失敗を記録するエラーログは、誰の、どの操作に対する失敗かが分かる境界（ハンドラー、エントリーポイントなど最外郭の処理）で1回だけ出力する。下位ではエラーログを出力せず、エラーを上位へ伝播させる。伝播のたびに出力すると、同じ失敗が何行にも分かれて記録される。

監視、性能分析、監査のためのイベントログはこの制限の対象外とし、出来事ごとに記録する。タスクの開始と完了、通信の開始、完了、失敗、センサーの異常などが該当する。
...
| 区分 | 項目 |
| --- | --- |
| 必須 | 時刻、レベル、イベント名、実行単位の名前とバージョン、実行環境 |
| 識別 | 実行主体と処理を識別する値。トレースID、リクエストID、セッションID、利用者ID、テナントID、デバイスID、ノードID、タスクIDなど |
| 結果 | 所要時間、件数、応答コードなど、処理の結果を表す値 |

時刻はタイムゾーンを含めて記録する。実時刻を取得できない環境では、起動からの経過時間と、起動ごとに変わる識別子を記録する。
