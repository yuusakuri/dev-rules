...
Composition Rootが多くの依存関係を構成し、一つの起動処理では依存関係を追跡しにくくなる場合は、接続先、外部資源、依存先が扱う領域ごとのモジュールや生成関数へ分割する。起動処理は共有する外部資源を生成して各構成単位を呼び出し、各構成単位は必要な依存関係を引数で受け取り、構築済みの公開境界を返す。構成単位には業務ロジックを持たせない。

依存先をFeatureなどの依存元ごとにグループ化することは避け、必要とするオブジェクトや処理へ個別に明示的に渡す。各依存元や依存先の構築には、必要に応じてBuilderやFactoryを使用する。
