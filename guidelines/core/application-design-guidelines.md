...
セキュリティの設計は、次の3つの文書に従う。上位の文書で満たすべき水準を決め、下位の文書でその水準を満たす実装方法を決めることで、要求と実装が食い違わないようにする。
...
リクエストを受信した境界で認証情報を検証し、利用者を確定する。検証に失敗したリクエストは業務処理へ渡さない。認証情報の保存、多要素認証、認証失敗時の応答は、Authentication Cheat Sheetに従う。
