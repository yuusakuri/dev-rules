...
| `<source-root>/infra/<technical-resource>/` | [`infra/logger`](https://github.com/juspay/hyperswitch/tree/main/crates/router_env/src/logger)、[`infra/postgres/pool`](https://github.com/launchbadge/sqlx/tree/main/sqlx-core/src/pool)、`infra/supabase` | Featureの型や業務ルールに依存しないロガー、DB接続プール、外部プラットフォームのクライアント、Crash Reportingなどの構築処理を、技術的な資源ごとに配置し、起動点から呼び出す。ディレクトリには、`postgres`、`supabase`、`sentry`のような実際の製品、サービス、プロトコル、または`logging`のような技術的機能を表す名前を使用し、同じ仕組みで接続先が複数ある場合は起動点で設定を与えて生成し分ける。ネットワーク越しの供給であっても業務上の意味を持たないものはここへ置き、Feature固有の意味を持つ操作はFeature内の接続先ごとのディレクトリへ配置する。 |
