...
依存する機能やオブジェクトは、関数の引数または型のフィールドとして外部から明示的に渡す依存性の注入を採用する。グローバル状態やサービスロケータのように依存関係を隠す仕組みは、処理の理解とテストを難しくするため避ける。

依存関係の生成と接続は、Composition Rootとして各アプリケーションの起動処理に集約し、業務処理から分離する。これにより、アプリケーション全体の構成と実装の選択を一か所で確認でき、変更すべき部分を特定しやすくなり、テスト用の実装へも差し替えやすくなる。

構築は基本的に手動DI（Composition Rootでの直接的な生成と受け渡し）で行う。依存グラフが大きくなり、手動での構築コード自体が保守対象として複雑化する場合は、DIフレームワークの採用を検討する。フレームワークを採用する場合も、依存関係を隠すグローバルな解決の仕組みとしてではなく、Composition Rootでの構築を機械化する手段として使う。
...
共通化するかどうかは、出現回数ではなく重複しているコードが同じ概念、同じ業務知識を表しているかで判断する。
...
モジュールは直接の協力者とだけ通信し、協力者を踏み台にしてその先の別オブジェクトを操作しない。`a.getB().getC().doSomething()` のように、B を経由して無関係な C を操作する連鎖は、A が B と C の内部構造を知りすぎており、B や C の変更が A に波及する。Fluent APIやコレクション操作の`filter().map()`のように、同じ対象への操作を返し続けるメソッドチェーンはこの原則の対象外とする。
