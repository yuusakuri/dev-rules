...
| 実装 | [通信プロトコル実装ガイドライン](implementation/communication-protocol-guidelines.md) | ネットワーク通信、シリアル通信、CANなどの通信路で、フレーム、ストリーム解析、誤り検出、性能及び復帰処理を設計するためのガイドライン。 | ソフトウェア設計ガイドライン。アプリケーションの一部として実装する場合はアプリケーション設計ガイドライン、該当する実装ガイドラインも読む |
