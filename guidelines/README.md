...
| 分類 | 文書 | 概要 | 先に読む文書 |
| --- | --- | --- | --- |
| 共通 | [共通設計原則](core/software-design-guidelines.md) | 責務の分離、依存関係の管理、複雑性の制御、名前空間・型名・メソッド名の重複を避ける命名など、ソフトウェア開発に共通する設計原則。 | なし |
...
名前空間、型名、メソッド名を設計または見直す場合は、共通設計原則の「命名」を読む。実装を伴う場合は、アプリケーション設計規則と対象言語の規則も併用する。
