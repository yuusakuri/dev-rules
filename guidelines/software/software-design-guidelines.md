...
モジュール、ディレクトリ、型への責務の分け方に迷う場合は、自分たちだけの判断で決めず、同じ領域、同じ種類のソフトウェアで広く使われているOSSが責務をどう分割しているかを確認し、参考にする。実績のあるOSSの構造は、多くの利用者と変更を経て磨かれており、独自に考えるより境界の妥当性が検証されている。
...
| ルール | 内容 |
| --- | --- |
| 有名OSSを参照する | 命名に迷う場合や、本書の「単語」に記載がない概念を命名する場合は、対象言語の標準ライブラリや、同じ領域で広く使われているOSSの命名を確認し、それに倣う。独自の造語より、実績のある名前を優先する。 |
